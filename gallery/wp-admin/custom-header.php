...
class Custom_Image_Header {
	 ...
	 * @since 3.0.0
	 *
	 * @param string $type The header type. One of 'default' (for the Uploaded Image control)
	 *                     or 'uploaded' (for the Uploaded Image control).
	 */
...
<td>
	<p><?php _e( 'You can select an image to be shown at the top of your site by uploading from your computer or choosing from your media library. After selecting an image you will be able to crop it.' ); ?><br />
			<?php
			if ( ! current_theme_supports( 'custom-header', 'flex-height' ) && ! current_theme_supports( 'custom-header', 'flex-width' ) ) {
				printf( __( 'Image of exactly <strong>%1$d &times; %2$d pixels</strong> will be used as-is.' ) . '<br />', get_theme_support( 'custom-header', 'width' ), get_theme_support( 'custom-header', 'height' ) );
			} elseif ( current_theme_supports( 'custom-header', 'flex-height' ) ) {
				if ( ! current_theme_supports( 'custom-header', 'flex-width' ) ) {
					printf(
						/* translators: %s: size in pixels */
						__( 'Image should be at least %s wide.' ) . ' ',
						sprintf(
				...
				if ( ! current_theme_supports( 'custom-header', 'flex-height' ) ) {
					printf(
						/* translators: %s: size in pixels */
						__( 'Image should be at least %s tall.' ) . ' ',
						sprintf(
...
<tr>
<th scope="row"><?php _e( 'Uploaded Image' ); ?></th>
<td>
	<p><?php _e( 'You can choose one of your previously uploaded headers, or show a random one.' ); ?></p>
		...
		if ( ! empty( $this->default_headers ) ) :
			?>
<tr>
<th scope="row"><?php _e( 'Default Image' ); ?></th>
<td>

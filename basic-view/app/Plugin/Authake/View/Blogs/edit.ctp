...
<div id="content">
	<div class="container">
	 ...
	 <a href="<?php echo $this->Html->url( array('controller'=> 'blogs', 'action'=>'index')); ?>" class="btn btn-link">Cancel</a>
					<?php echo $this->Html->link(__('Delete'), array('action'=>'delete', $this->Form->value('Blog.id')), array('class'=>'btn btn-danger'), sprintf(__('Are you sure you want to delete @%s?'), $this->Form->value('Blog.id'))); ?>
				</div>
			</div>
		  ...
		  <?php echo $this->Form->create('Blog',array('enctype'=>"multipart/form-data"));?>
			...
			<?php echo $this->Html->link(__('View News'), array('action'=>'index', $this->Form->value('Blog.id')), array('class'=>'btn btn-primary'));?>

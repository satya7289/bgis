...
class AuthakeComponent extends Component {
    ...
    function startup(Controller $controller = null) {
        ...
        if (Configure::read('Authake.baseUrl') == null) {
            Configure::write('Authake.baseUrl', Router::url('/authake/users', true));   // set the full application url
        }
		...
		/**
         * Used to redirect the users if the current user is front desk user
         */
        if (Configure::read('Authake.resultDeskAction') == null) {
            Configure::write('Authake.resultDeskAction', array('plugin' => 'authake', 'controller' => 'results', 'action' => 'index'));
        }
        /**

...
$this->Session->setFlash(__('You are logged in as ').$userdata['User']['login'], 'success' , array('plugin'=>'authake'));
				//echo $currGroup;die;
					if($userdata['Group'][0]['name'] == "Administrators")
					{
					  $this->redirect('/authake/');
				    }
			    	  /*   elseif($userdata['Group'][0]['name'] == "Data Entry")
			 	    {
					$this->redirect(Configure::read('Authake.frontDeskAction'));
				    }
					*/
				     elseif($userdata['Group'][0]['name'] == "Result Admin")
			 	    {
					    $this->redirect(Configure::read('Authake.resultDeskAction'));
				    }
					 else
					{
						 if(!isEmpty($this->Session->read('Authake.id')))
						  { 
